...
<form method="POST" action="/techaasvik_admin/course/settings/save" enctype="multipart/form-data">
  ...
  <div class="admin-table-wrapper" style="max-width:600px;padding:24px;">
    ...
    <div style="background:<?= $coursesEnabled ? 'rgba(52,211,153,0.06)' : 'rgba(248,113,113,0.06)' ?>;border:1px solid <?= $coursesEnabled ? 'rgba(52,211,153,0.25)' : 'rgba(248,113,113,0.25)' ?>;border-radius:14px;padding:24px;margin-bottom:28px;display:flex;align-items:center;justify-content:space-between;gap:20px;">
      ...
      <label style="position:relative;display:inline-block;width:60px;height:32px;flex-shrink:0;">
        ...
        <span style="position:absolute;cursor:pointer;inset:0;background:<?= $coursesEnabled ? '#34d399' : '#374151' ?>;border-radius:34px;transition:0.3s;">
          <span style="position:absolute;height:24px;width:24px;left:<?= $coursesEnabled ? '32px' : '4px' ?>;bottom:4px;background:#fff;border-radius:50%;transition:0.3s;"></span>
        </span>
      </label>
    </div>
    ...
    <div style="background:rgba(251,191,36,0.06);border:1px solid rgba(251,191,36,0.2);border-radius:10px;padding:14px 18px;font-size:13px;color:#fbbf24;">
      ⚠️ <strong>Warning:</strong> Disabling courses will immediately hide them from all visitors. Students with existing enrollments will not be able to access their course content until you re-enable.
    </div>

    <?php // Hidden fields to preserve other tab values ?>
    <input type="hidden" name="courses_enabled" id="coursesEnabledHidden" value="<?= $coursesEnabled ? '1' : '0' ?>">
    <?php // carry-forward all other settings as hidden: ?>
...
function toggleCoursesEnabled(checkbox) {
  ...
  const hidden = document.getElementById('coursesEnabledHidden');
  if (hidden) hidden.value = val;
  // Auto-submit the form so the change saves immediately
