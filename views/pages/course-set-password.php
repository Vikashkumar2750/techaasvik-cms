<?php
/* course-set-password.php — user sets password after clicking verification link */
use Core\Auth;
Auth::startSession();
...
<div class="auth-wrap">
  <div class="auth-card">
    ...
    <p class="auth-sub">Email: <strong style="color:var(--text-primary);"><?= htmlspecialchars($email) ?></strong></p>

    <?php if ($error): ?>
    <div class="auth-error">⚠️ <?= htmlspecialchars($error) ?></div>
    <?php endif; ?>

    <form method="POST" action="/courses/set-password" id="setPasswordForm">
      <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrfToken) ?>">

      <div class="auth-field">
        <label class="auth-label" for="sp-password">New Password</label>
        <input type="password" id="sp-password" name="password" class="auth-input" placeholder="Minimum 8 characters" required minlength="8" oninput="checkStrength(this.value)">
      ...
      <div class="auth-field">
        <label class="auth-label" for="sp-confirm">Confirm Password</label>
        <input type="password" id="sp-confirm" name="confirm_password" class="auth-input" placeholder="Re-enter your password" required>
      </div>

      <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:14px;font-size:15px;margin-top:8px;" id="spSubmit">
        Set Password & Start Learning →
      </button>
    </form>
  </div>
</div>
...
document.getElementById('setPasswordForm').onsubmit = function(e) {
  const p = document.getElementById('sp-password').value;
  const c = document.getElementById('sp-confirm').value;
  if (p !== c) { e.preventDefault(); alert('Passwords do not match.'); return; }
  document.getElementById('spSubmit').textContent = 'Setting password...';
  document.getElementById('spSubmit').disabled = true;
};
</script>
