...
class SeoToolsController extends Controller
{
    ...
    public function seedPillars(array $params = []): void
    {
        ...
        try {
            ...
            if (!$authorId) { echo "ERROR: No active author found.\n"; return; }

            $seedFile = APP_ROOT . '/seed-pillars-data.php';
            if (!file_exists($seedFile)) { echo "ERROR: seed-pillars-data.php not found\n"; return; }

            $pillars = require $seedFile;
            echo "Found " . count($pillars) . " pillars\n\n";
            ...
            foreach ($pillars as $p) {
                ...
                $contentId = $db->insert('content', [
                    ...
                    'status' => 'published', 'author_id' => $authorId,
                    'word_count' => $p['word_count'], 'read_time' => $p['read_time'],
                    'difficulty' => $p['difficulty'], 'published_at' => date('Y-m-d H:i:s'),
                ]);
