...
class SchemaService
{
    ...
    public function person(array $author): array
    {
        return [
            ...
            'image'           => !empty($author['photo_url']) ? $author['photo_url'] : null,
            'worksFor'        => ['@id' => $this->siteUrl . '/#organization'],
            'sameAs'          => array_filter([
    ...
    // ── DefinedTerm (Glossary) ───────────────────────────
    public function definedTerm(array $term, string $url): array
    {
        return [
            '@context'         => 'https://schema.org',
            '@type'            => 'DefinedTerm',
            'name'             => $term['title'],
            'description'      => strip_tags($term['excerpt'] ?? $term['content'] ?? ''),
            'url'              => $url,
            'inDefinedTermSet' => [
                '@type' => 'DefinedTermSet',
                'name'  => 'Digital Marketing Glossary',
    ...
    // ── SoftwareApplication (Tool) ───────────────────────
    public function softwareApp(array $tool): array
    {
        return [
            '@context'            => 'https://schema.org',
            '@type'               => 'SoftwareApplication',
            'name'                => $tool['title'],
            'description'         => strip_tags($tool['excerpt'] ?? ''),
            'url'                 => $this->siteUrl . '/tools/' . $tool['slug'],
    ...
    // ── Course ───────────────────────────────────────────
    public function course(array $course): array
    {
        return [
            '@context'    => 'https://schema.org',
            '@type'       => 'Course',
            'name'        => $course['title'],
            'description' => strip_tags($course['excerpt'] ?? ''),
